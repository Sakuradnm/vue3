...
import { applyLecturer } from '@/api/notification'
...
const showApplyLecturerModal = ref(false)
const lecturerApplicationReason = ref('')
...
// 申请成为讲师
const handleApplyLecturer = async () => {
  if (!lecturerApplicationReason.value.trim()) {
    ElMessage.warning('请填写申请理由')
    return
  }
  
  try {
    await applyLecturer(userId.value, lecturerApplicationReason.value)
    showApplyLecturerModal.value = false
    lecturerApplicationReason.value = ''
...
<template>
  <div class="pc-root">
      ...
      <main class="main-panel">
        ...
        <section v-if="activeSection === 'security'" class="panel">
          ...
          <div class="security-list">
            ...
            <!-- 仅学生用户显示申请成为讲师 -->
            <div v-if="userInfo.level === 'student'" class="security-item">
              <div class="si-icon lecturer">🎓</div>
              <div class="si-info">
                <p class="si-title">申请成为讲师</p>
                <p class="si-desc">申请后管理员将审核您的资质，通过后可上传课程</p>
              </div>
              <button class="btn-primary sm" @click="showApplyLecturerModal = true">立即申请</button>
            </div>
          </div>
        </section>
      </main>
    ...
    <!-- 申请成为讲师弹窗 -->
    <Transition name="modal-fade">
      <div v-if="showApplyLecturerModal" class="modal-overlay" @click.self="showApplyLecturerModal = false">
        <div class="modal">
          <div class="modal-header">
            <h3>申请成为讲师</h3>
            <button class="modal-close" @click="showApplyLecturerModal = false">✕</button>
          </div>
          <div class="modal-body">
            <div class="form-group">
              <label>申请理由 <span style="color: #f56c6c;">*</span></label>
              <textarea 
                v-model="lecturerApplicationReason" 
                rows="5" 
                placeholder="请说明您的专业背景、教学经验等（最多500字）"
                maxlength="500"
              />
              <p class="char-count">{{ lecturerApplicationReason.length }}/500</p>
            </div>
            <el-alert
              title="提示"
              type="info"
              description="提交申请后，管理员将在1-3个工作日内审核。审核结果将通过消息中心通知您。"
              :closable="false"
              show-icon
            />
          </div>
          <div class="modal-footer">
            <button class="btn-ghost" @click="showApplyLecturerModal = false">取消</button>
            <button class="btn-primary" @click="handleApplyLecturer">提交申请</button>
          </div>
        </div>
      </div>
    </Transition>
...
.si-icon.lecturer { background: rgba(245,108,108,0.1); }
